// ==UserScript==
// @name         Tapas,Webtoon,Mangatoon,Toomics,Naver,Dongmanmanhua,Lalatoon,Honeytoon,Toptoon Ripper(All Languages)
// @namespace    http://tampermonkey.net/
// @version      1.0.3
// @description  One click download for 9 sites mentioned and their language variants
// @author       ozler365
// @license      MIT
...
// @match        https://it.mangatoon.mobi/*
// @match        https://ar.mangatoon.mobi/*
// @match        https://de.mangatoon.mobi/*
// @match        https://fr.mangatoon.mobi/*
// @match        https://tapas.io/*
// @match        https://www.webtoons.com/*
// @match        https://www.dongmanmanhua.cn/*
// @match        https://comic.naver.com/*
// @match        https://global.lalatoon.com/*
// @match        https://lalatoon.com/*
// @match        https://honeytoon.com/*
// @match        https://global.toptoon.com/*
// @match        https://www.toptoon.net/*
// @match        https://toptoon.com/*
// @match        https://mangatooncom.vn/*
// @grant        GM_download
// @grant        GM_xmlhttpRequest
// @downloadURL https://update.greasyfork.org/scripts/594644/Tapas%2CWebtoon%2CMangatoon%2CToomics%2CNaver%2CDongmanmanhua%2CLalatoon%2CHoneytoon%2CToptoon%20Ripper%28All%20Languages%29.user.js
// @updateURL https://update.greasyfork.org/scripts/594644/Tapas%2CWebtoon%2CMangatoon%2CToomics%2CNaver%2CDongmanmanhua%2CLalatoon%2CHoneytoon%2CToptoon%20Ripper%28All%20Languages%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    const siteConfigs = [
        ...
        { host: 'mangatooncom.vn', container: 'body', imgSelector: 'img.lazyload_img, img', attrs: ['data-src', 'src'] },
        ...
        { host: 'dongmanmanhua.cn', container: 'body', imgSelector: 'img._images', attrs: ['data-url', 'src'] },
        { host: 'comic.naver.com', container: 'body', imgSelector: 'img[id^="content_image_"]', attrs: ['src'] },
        { host: 'lalatoon.com', container: 'body', imgSelector: 'img[id^="set_image_"]', attrs: ['data-original', 'src'] },
        { host: 'honeytoon.com', container: 'body', imgSelector: '.single__item img, img.comic-single__image', attrs: ['data-src', 'src'] },
        { host: 'global.toptoon.com', container: 'body', imgSelector: 'img[data-index]', attrs: ['data-src', 'src'] },
        { host: 'toptoon.net', container: 'body', imgSelector: '.cImg img', attrs: ['data-src', 'src'] },
        // Updated Toptoon.com to capture both img and div variants
        { host: 'toptoon.com', container: 'body', imgSelector: '.document_img', attrs: ['data-src', 'src'] }
    ];
        ...
        containerNodes.forEach((containerNode) => {
            ...
            images.forEach((img, index) => {
                ...
                if (targetSrc && !downloadedUrls.has(targetSrc)) {
                    ...
                    else if (config.host === 'mangatooncom.vn') {
                        const altText = img.getAttribute('alt') || '';
                        const match = altText.match(/-\s*(\d+)\s*$/);
                        if (match && match[1]) {
                            fileName = `image_${String(parseInt(match[1]) + 1).padStart(3, '0')}.png`;
                        }
                    }
                    else if (config.host === 'comic.naver.com') {
                        const idText = img.getAttribute('id') || '';
                        const match = idText.match(/content_image_(\d+)/);
                        if (match && match[1]) {
                            fileName = `image_${match[1].padStart(3, '0')}.png`;
                        }
                    }
                    else if (config.host === 'lalatoon.com') {
                        const idText = img.getAttribute('id') || '';
                        const match = idText.match(/set_image_(\d+)/);
                        if (match && match[1]) {
                            fileName = `image_${String(parseInt(match[1]) + 1).padStart(3, '0')}.png`;
                        }
                    }
                    else if (config.host === 'honeytoon.com') {
                        const altText = img.getAttribute('alt') || '';
                        const match = altText.match(/scene\s*(\d+)/i);
                        if (match && match[1]) {
                            fileName = `image_${match[1].padStart(3, '0')}.png`;
                        }
                    }
                    else if (config.host === 'global.toptoon.com') {
                        const dataIndex = img.getAttribute('data-index');
                        if (dataIndex !== null) {
                            fileName = `image_${String(parseInt(dataIndex) + 1).padStart(3, '0')}.png`;
                        }
                    }
                    else if (config.host === 'toptoon.com') {
                        // Looks for the index in either 'alt' (for the initial img) or 'data-title' (for the divs)
                        const titleText = img.getAttribute('alt') || img.getAttribute('data-title') || '';
                        const match = titleText.match(/_(\d+)$/);
                        if (match && match[1]) {
                            fileName = `image_${String(parseInt(match[1]) + 1).padStart(3, '0')}.png`;
                        }
                    }
    ...
    // --- Specific Auto-Scroll for mangatooncom.vn ---
    async function smartAutoScroll() {
        const btn = document.getElementById('gm-autoscroll-btn');
        if (btn) { btn.innerText = 'Scrolling...'; btn.disabled = true; }

        const images = document.querySelectorAll('img.lazyload_img');
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            await new Promise((resolve) => {
                ...
                const checkInterval = setInterval(() => {
                    ...
                    const isDefaultSrc = src.includes('content_cover_default.png');
                    const isDefaultData = dataSrc.includes('content_cover_default.png');
                    
                    const hasRealUrl = (src && !isDefaultSrc) || (dataSrc && !isDefaultData);

                    if (hasRealUrl || attempts > 150) { 
        ...
        if (btn) { btn.innerText = 'All Loaded!'; }
        setTimeout(() => { 
            if (btn) { btn.innerText = 'Auto Scroll'; btn.disabled = false; } 
        }, 3000);
    }
    ...
    function createUI() {
        ...
        // Conditionally append Auto-Scroll button just for mangatooncom.vn
        if (window.location.hostname.includes('mangatooncom.vn')) {
            const autoScrollBtn = document.createElement('button');
            autoScrollBtn.id = 'gm-autoscroll-btn';
            autoScrollBtn.innerText = 'Auto Scroll';
            autoScrollBtn.style.cssText = `
                padding: 10px 10px; cursor: pointer; background: #007bff; color: white; 
                border: none; border-radius: 4px; font-weight: bold; font-size: 13px;
                width: 100%; margin-top: 6px;
            `;
            autoScrollBtn.onclick = smartAutoScroll;
